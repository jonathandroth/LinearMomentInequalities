function [m_theta,Ceq,Dm_theta,DCeq] = KMS_22_EAM_FeasibleSearch_constraint(theta_aug,f_ineq,f_eq,f_ineq_keep,f_eq_keep,f_stdev_ineq,f_stdev_eq,dmodel,KMSoptions)
...
% the ith moment minus the dace approximated c(theta).
%
% INPUT:
...
if nargout > 2
    ...
    Dm_theta = sqrt(n).*Dg./repmat([f_stdev_ineq;f_stdev_eq],1,dim_p) + repmat(dc_theta.',[J,1]);
